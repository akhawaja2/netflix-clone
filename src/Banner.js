...
function Banner() {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            var randomInt= Math.floor(Math.random() * request.data.results.length);
    ...
    console.log(movie);
    //function to only show x characters before '...'like netflix desc.
    function truncate(str, n)
    {
        return str?.length > n ? str.substr(0, n-1) + "..." : str;
    }
    //Since banner is on top it is a header
    return (
